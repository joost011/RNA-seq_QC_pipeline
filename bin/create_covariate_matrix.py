...
def get_all_metrics(sample, input_dir):
    ...
    columns, values = get_alignment_metrics(f'{input_dir}/multiple_metrics/{sample}/multiple_metrics.alignment_summary_metrics.gz', columns, values)
    columns, values = get_insert_metrics(f'{input_dir}/multiple_metrics/{sample}/multiple_metrics.insert_size_metrics.gz', columns, values)
    columns, values = get_rna_seq_metrics(f'{input_dir}/rna_seq_metrics/{sample}/{sample}_rnaseqmetrics.gz', columns, values)
    columns, values = get_star_log_metrics(f'{input_dir}/star/{sample}/{sample}_Log.final.out.gz', columns, values)
    columns, values = get_star_count_metrics(f'{input_dir}/star/{sample}/{sample}_ReadsPerGene.out.tab.gz', columns, values)
...
if __name__ == '__main__':
    ...
    for sample in os.listdir(f'{input_directory}/star'):
        try:
            col, val = get_all_metrics(sample, input_directory)
            columns = col
            values.append(val)
